...
bool useProgramSlit(const std::string &mode_raw)
{
  const std::string mode = normalizeModeToken(mode_raw);
  return mode == "program_slit" || mode == "slit" || mode == "z_slit" || mode == "low_profile";
}
...
public:
  void init()
  {
    ...
    loadDoubleParam(pnh, nh, "slit_spread_factor",
                    "/swarm_master_node/swarm_master/slit_spread_factor",
                    2.5, slit_spread_factor_cmd_);
    loadDoubleParam(pnh, nh, "payload_rope_length",
                    "/swarm_master_node/swarm_master/payload/rope_length",
                    1.0, payload_rope_length_);
    loadDoubleParam(pnh, nh, "payload_template_rope_margin",
                    "/swarm_master_node/swarm_master/payload/template_rope_margin",
                    0.05, payload_template_rope_margin_);
    ...
    pnh.param("slit_extra_rope_margin", slit_extra_rope_margin_, 0.05);
    pnh.param("slit_enforce_rope_feasibility", slit_enforce_rope_feasibility_, false);
    pnh.param("slit_min_spread_factor", slit_min_spread_factor_, 1.0);
    ...
    pnh.param("transition_duration", transition_duration_, 1.6);
    ...
    if (!pnh.getParam("agent_ids", agent_ids_) || agent_ids_.empty())
    {
      ...
      ROS_WARN("[ProgramSlit] agent_ids not provided, fallback to [1, 2].");
    }
    ...
    mode_sub_ = nh.subscribe(mode_topic_, 5, &SwarmProgramSlit::modeCallback, this);
    leader_guidance_sub_ = nh.subscribe(leader_guidance_topic_, 10,
                                        &SwarmProgramSlit::leaderGuidanceCallback, this);
    leader_state_sub_ = nh.subscribe(leader_state_topic_, 20,
                                     &SwarmProgramSlit::leaderStateCallback, this);
    ...
    for (size_t i = 0; i < agent_ids_.size(); ++i)
    {
      ...
      base_guidance_subs_.push_back(
          nh.subscribe<ego_planner::Bezier>(
              ...
              boost::bind(&SwarmProgramSlit::baseGuidanceCallback, this, _1, agent_id)));
      ...
      ROS_INFO("[ProgramSlit] agent=%d normal=%s output=%s rank=%d",
               agent_id,
    ...
    timer_ = nh.createTimer(ros::Duration(1.0 / std::max(1.0, publish_rate_)),
                            &SwarmProgramSlit::timerCallback, this);

    ROS_INFO("[ProgramSlit] ready. mode_topic=%s spread_cmd=%.3f spread_target=%.3f min_spread=%.3f enforce_rope=%d R0=%.3f R_cap=%.3f guard(rope=%.3f area=%.3f pair=%.3f samples_per_seg=%d)",
             mode_topic_.c_str(), slit_spread_factor_cmd_, spread_target_, slit_min_spread_factor_,
             static_cast<int>(slit_enforce_rope_feasibility_), template_radius_r0_, rope_radius_cap_,
...
private:
  ...
  void resolveSpreadTarget()
  {
    const double spread_cmd = std::max(0.0, slit_spread_factor_cmd_);
    const double spread_floor = std::max(0.0, slit_min_spread_factor_);

    double back_dist = 0.0;
    double lateral_dist = 0.0;
    resolveRankGeometry(1, back_dist, lateral_dist);

    const Eigen::Vector3d follower1(-back_dist, lateral_dist, formation_z_offset_);
    const Eigen::Vector3d follower2(-back_dist, -lateral_dist, formation_z_offset_);
    const auto circum = ego_planner::payload_geometry::computeCircumradiusAndGrad(
        Eigen::Vector3d::Zero(), follower1, follower2);

    if (!circum.valid || circum.circumradius < kEps)
    {
      template_radius_r0_ = 0.0;
      spread_target_ = std::max(spread_cmd, spread_floor);
      ROS_WARN("[ProgramSlit] failed to compute template circumradius, fallback spread_target=%.3f (enforce_rope=%d).",
               spread_target_, static_cast<int>(slit_enforce_rope_feasibility_));
      return;
    }

    template_radius_r0_ = circum.circumradius;
    rope_radius_cap_ = payload_rope_length_ - payload_template_rope_margin_ - slit_extra_rope_margin_;

    if (rope_radius_cap_ <= 0.0)
    {
      ROS_WARN("[ProgramSlit] rope feasibility cap non-positive (rope=%.3f template_margin=%.3f extra_margin=%.3f).",
               payload_rope_length_, payload_template_rope_margin_, slit_extra_rope_margin_);
    }

    const double spread_cap = rope_radius_cap_ / std::max(template_radius_r0_, kEps);
    if (slit_enforce_rope_feasibility_)
    {
      spread_target_ = std::min(spread_cmd, spread_cap);
      if (!std::isfinite(spread_target_))
        spread_target_ = spread_cmd;
      spread_target_ = std::max(0.0, spread_target_);

      if (spread_target_ + 1e-6 < spread_cmd)
      {
        ROS_WARN("[ProgramSlit] rope feasibility clamp active: spread_cmd=%.3f -> spread_target=%.3f "
                 "(R0=%.3f R_cap=%.3f, rope=%.3f template_margin=%.3f extra_margin=%.3f)",
                 spread_cmd, spread_target_, template_radius_r0_, rope_radius_cap_,
                 payload_rope_length_, payload_template_rope_margin_, slit_extra_rope_margin_);
      }

      if (spread_target_ + 1e-6 < spread_floor)
      {
        ROS_WARN("[ProgramSlit] spread_target=%.3f is below slit_min_spread_factor=%.3f due to strict rope feasibility.",
                 spread_target_, spread_floor);
      }
    }
    else
    {
      spread_target_ = std::max(spread_cmd, spread_floor);
      if (!std::isfinite(spread_target_))
        spread_target_ = std::max(1.0, spread_floor);

      if (std::isfinite(spread_cap) && spread_target_ > spread_cap + 1e-6)
      {
        ROS_WARN("[ProgramSlit] dynamic slit mode keeps spread_target=%.3f above static rope cap %.3f "
                 "(R0=%.3f, rope=%.3f, template_margin=%.3f, extra_margin=%.3f).",
                 spread_target_, spread_cap, template_radius_r0_, payload_rope_length_,
                 payload_template_rope_margin_, slit_extra_rope_margin_);
      }
    }

    const double angle_rad = formation_angle_deg_ * M_PI / 180.0;
    const double pair_sep = 2.0 * std::abs(formation_spacing_ * std::sin(angle_rad));
    const double pair_min = std::max(safe_radius_, inter_uav_sep_min_);
    if (pair_sep + 1e-3 < pair_min)
    {
      ROS_WARN("[ProgramSlit] nominal pair separation %.3f < safety threshold %.3f. "
               "Slit mode will follow provided spacing, verify formation params.",
               pair_sep, pair_min);
    }
  }
  ...
  void modeCallback(const std_msgs::StringConstPtr &msg)
  {
    ...
    target_active_ = useProgramSlit(msg->data);
    ROS_INFO("[ProgramSlit] mode command raw='%s' normalized='%s' -> target_active=%d",
  ...
  void buildSlitCtrl(const Eigen::MatrixXd &leader_ctrl,
                     const int rank,
                     const double alpha,
                     Eigen::MatrixXd &slit_ctrl) const
  {
    slit_ctrl = leader_ctrl;
    ...
    const double spread = 1.0 + alpha * (spread_target_ - 1.0);
    ...
    for (int i = 0; i < leader_ctrl.cols(); ++i)
    {
      ...
      const Eigen::Vector3d slit_offset =
          -forward * (back_dist * spread) +
          left_dir * (lateral_dist * spread) +
          Eigen::Vector3d(0.0, 0.0, formation_z_offset_);
      slit_ctrl.col(i) = leader_ctrl.col(i) + slit_offset;
    }
  }
  ...
  payload_guard::PayloadConstraintGuardParams makePayloadGuardParams() const
  {
    ...
    return params;
	  ...
	  void publishAgentGuidance(const int agent_id, const ros::Time &now)
	  {
	    ...
	    if (base_ctrl.cols() != leader_ctrl.cols())
	    {
	      ROS_WARN_THROTTLE(1.0,
	                        "[ProgramSlit] agent=%d ctrl size mismatch: base=%d leader=%d, skip.",
      ...
      Eigen::MatrixXd slit_ctrl;
      buildSlitCtrl(leader_ctrl, rank_it->second, alpha, slit_ctrl);

      const Eigen::MatrixXd out_ctrl = slit_ctrl;
	    ...
	    ROS_INFO_THROTTLE(0.5,
	                      "[ProgramSlit] agent=%d publish alpha=%.2f spread=%.3f traj_id=%ld start_in=%.3f",
	                      agent_id, alpha, 1.0 + alpha * (spread_target_ - 1.0),
	                      static_cast<long>(out.traj_id),
	  ...
	  void publishGuardedPairGuidance(const ros::Time &now)
	  {
	    ...
	    {
	      ROS_WARN_THROTTLE(1.0,
	                        "[ProgramSlit] guarded pair ctrl size mismatch: leader=%d base1=%d base2=%d, skip.",
	                        static_cast<int>(leader_ctrl.cols()),
      ...
      Eigen::MatrixXd slit_ctrl1;
      Eigen::MatrixXd slit_ctrl2;
      buildSlitCtrl(leader_ctrl, rank_it1->second, alpha1, slit_ctrl1);
      buildSlitCtrl(leader_ctrl, rank_it2->second, alpha2, slit_ctrl2);
      ...
      const auto guard_result = payload_guard::projectPayloadConstraintTrajectoryBlend(
          leader_ctrl, base_ctrl1, base_ctrl2, slit_ctrl1, slit_ctrl2,
      ...
      const Eigen::MatrixXd out_ctrl1 =
          base_ctrl1 + guard_result.blend * (slit_ctrl1 - base_ctrl1);
      const Eigen::MatrixXd out_ctrl2 =
          base_ctrl2 + guard_result.blend * (slit_ctrl2 - base_ctrl2);

      if (guard_result.blend + 1e-4 < 1.0)
      {
        ROS_WARN_THROTTLE(1.0,
                          "[ProgramSlit] payload guard limited trajectory blend to %.2f (R_max=%.3f, A_min=%.3f, pair_min=%.3f, samples=%d).",
                          guard_result.blend,
      ...
      if (!guard_result.trajectory.valid)
      {
        ROS_ERROR_THROTTLE(1.0,
                           "[ProgramSlit] payload guard could not find a feasible trajectory blend; fallback stays at base guidance there.");
      }
  ...
  void timerCallback(const ros::TimerEvent &event)
  {
    ...
    const double old_alpha = slit_alpha_;
    if (target_active_)
      slit_alpha_ = clamp01(slit_alpha_ + dt / std::max(1e-3, transition_duration_));
    else
      slit_alpha_ = clamp01(slit_alpha_ - dt / std::max(1e-3, transition_duration_));

    const bool alpha_moved = std::fabs(slit_alpha_ - old_alpha) > 1e-4;
...
private:
  ...
  double inter_uav_sep_min_{0.8};
	  ...
	  double slit_spread_factor_cmd_{2.5};
	  double payload_rope_length_{1.0};
	  double payload_template_rope_margin_{0.05};
	  double payload_triangle_area_min_{0.05};
	  double slit_extra_rope_margin_{0.05};
	  bool slit_enforce_rope_feasibility_{false};
  double slit_min_spread_factor_{1.0};
  ...
  double spread_target_{2.5};
  double template_radius_r0_{0.0};
  double rope_radius_cap_{0.0};

  double transition_duration_{1.6};
  ...
  double slit_alpha_{0.0};
  int64_t custom_traj_id_{400000};
...
{
  ros::init(argc, argv, "swarm_program_slit_node");

  SwarmProgramSlit node;
  node.init();
  ros::spin();
  return 0;
}
