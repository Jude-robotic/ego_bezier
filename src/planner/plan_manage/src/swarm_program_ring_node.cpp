...
bool useProgramRing(const std::string &mode_raw)
{
  const std::string mode = normalizeModeToken(mode_raw);
  return mode == "program_ring" || mode == "ring" || mode == "line" || mode == "custom_ring";
}
...
public:
  void init()
  {
    ...
    pnh.param("compact_lateral_scale", compact_lateral_scale_, 0.35);
    ...
    if (!pnh.getParam("agent_ids", agent_ids_) || agent_ids_.empty())
    {
      ...
      ROS_WARN("[ProgramRing] agent_ids not provided, fallback to [1, 2].");
    }

    if (!std::isfinite(compact_lateral_scale_))
      compact_lateral_scale_ = 0.35;
    compact_lateral_scale_ = std::max(0.0, std::min(1.0, compact_lateral_scale_));
    payload_constraint_samples_per_seg_ =
        std::max(1, static_cast<int>(std::round(payload_constraint_samples_per_seg_double_)));

    mode_sub_ = nh.subscribe(mode_topic_, 5, &SwarmProgramRing::modeCallback, this);
    leader_guidance_sub_ = nh.subscribe(leader_guidance_topic_, 10,
                                        &SwarmProgramRing::leaderGuidanceCallback, this);
    leader_state_sub_ = nh.subscribe(leader_state_topic_, 20,
                                     &SwarmProgramRing::leaderStateCallback, this);
    ...
    for (size_t i = 0; i < agent_ids_.size(); ++i)
    {
      ...
      base_guidance_subs_.push_back(
          nh.subscribe<ego_planner::Bezier>(
              ...
              boost::bind(&SwarmProgramRing::baseGuidanceCallback, this, _1, agent_id)));
      ...
      ROS_INFO("[ProgramRing] agent=%d normal=%s output=%s rank=%d",
               agent_id,
    ...
    timer_ = nh.createTimer(ros::Duration(1.0 / std::max(1.0, publish_rate_)),
                            &SwarmProgramRing::timerCallback, this);

    ROS_INFO("[ProgramRing] ready. mode_topic=%s line_spacing=%.2f compact_lateral_scale=%.2f transition_duration=%.2fs guard(rope=%.3f area=%.3f pair=%.3f samples_per_seg=%d)",
             mode_topic_.c_str(), line_spacing_, compact_lateral_scale_, transition_duration_,
...
private:
  void modeCallback(const std_msgs::StringConstPtr &msg)
  {
    ...
    target_active_ = useProgramRing(msg->data);
    ROS_INFO("[ProgramRing] mode command raw='%s' normalized='%s' -> target_active=%d",
  ...
  Eigen::MatrixXd buildCompactRingCtrl(const Eigen::MatrixXd &leader_ctrl,
                                       const int rank,
                                       const double alpha) const
  {
    Eigen::MatrixXd ring_ctrl = leader_ctrl;
    ...
    const double ring_back = line_spacing_ * static_cast<double>(level);
    const double lateral_scale =
        (1.0 - alpha) + alpha * compact_lateral_scale_;
    const double eff_back = back_dist * (1.0 - alpha) + ring_back * alpha;
    ...
    for (int i = 0; i < leader_ctrl.cols(); ++i)
    {
      ...
      const Eigen::Vector3d ring_offset =
          -forward * eff_back +
          left_dir * eff_lateral +
          Eigen::Vector3d(0.0, 0.0, formation_z_offset_ * (1.0 - alpha));
      ring_ctrl.col(i) = leader_ctrl.col(i) + ring_offset;
    }
  ...
  payload_guard::PayloadConstraintGuardParams makePayloadGuardParams() const
  {
    ...
    return params;
	  ...
	  void publishAgentGuidance(const int agent_id, const ros::Time &now)
	  {
	    ...
	    if (base_ctrl.cols() != leader_ctrl.cols())
	    {
	      ROS_WARN_THROTTLE(1.0,
	                        "[ProgramRing] agent=%d ctrl size mismatch: base=%d leader=%d, skip.",
      ...
      const Eigen::MatrixXd ring_ctrl =
          buildCompactRingCtrl(leader_ctrl, rank_it->second, alpha);
      const Eigen::MatrixXd out_ctrl = ring_ctrl;
      ...
      ROS_INFO_THROTTLE(0.5,
                        "[ProgramRing] agent=%d publish alpha=%.2f compact=%.2f spacing=%.2f traj_id=%ld start_in=%.3f",
                        agent_id, alpha, compact_lateral_scale_, line_spacing_,
	  ...
	  void publishGuardedPairGuidance(const ros::Time &now)
	  {
	    ...
	    {
	      ROS_WARN_THROTTLE(1.0,
	                        "[ProgramRing] guarded pair ctrl size mismatch: leader=%d base1=%d base2=%d, skip.",
	                        static_cast<int>(leader_ctrl.cols()),
      ...
      const Eigen::MatrixXd ring_ctrl1 =
          buildCompactRingCtrl(leader_ctrl, rank_it1->second, alpha1);
      const Eigen::MatrixXd ring_ctrl2 =
          buildCompactRingCtrl(leader_ctrl, rank_it2->second, alpha2);
      ...
      const auto guard_result = payload_guard::projectPayloadConstraintTrajectoryBlend(
          leader_ctrl, base_ctrl1, base_ctrl2, ring_ctrl1, ring_ctrl2,
      ...
      const Eigen::MatrixXd out_ctrl1 =
          base_ctrl1 + guard_result.blend * (ring_ctrl1 - base_ctrl1);
      const Eigen::MatrixXd out_ctrl2 =
          base_ctrl2 + guard_result.blend * (ring_ctrl2 - base_ctrl2);

      if (guard_result.blend + 1e-4 < 1.0)
      {
        ROS_WARN_THROTTLE(1.0,
                          "[ProgramRing] payload guard limited trajectory blend to %.2f (%d invalid sample(s) at desired, R_max=%.3f, A_min=%.3f, pair_min=%.3f, samples=%d).",
                          guard_result.blend,
                          guard_result.desired_valid ? 0 : guard_result.trajectory.invalid_samples,
      ...
      if (!guard_result.trajectory.valid)
      {
        ROS_ERROR_THROTTLE(1.0,
                           "[ProgramRing] payload guard could not find a feasible trajectory blend; fallback stays at base guidance there.");
      }
...
private:
  ...
  double compact_lateral_scale_{0.35};
...
{
  ros::init(argc, argv, "swarm_program_ring_node");

  SwarmProgramRing node;
  node.init();
  ros::spin();
  return 0;
}
